import React from 'react';
import { Link } from 'react-router-dom';
import useCharacterDetail from '../../hooks/useHeyArnoldCharacterDetail';
import { useBEPost, useBEDelete } from '../../hooks/useBackend';

// eslint-disable-next-line max-len
const spinner = 'https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/945673bd-feaf-46f2-9e69-82deb505b95a/d9uqsyv-367865ee-f6f7-48ed-ba74-bbfa2a36a52a.gif?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..N-qjErA2OUW5Hz32FymO9LfJUBrnvqgY378NpbRSmbo';

export default function CharacterDetail() {

  const { character, loading } = useCharacterDetail();

  if(loading) {
    return (
      <img src={spinner} alt="loading gif" />
    );
  }

  return (
    <main>
      <header>
        <Link to="/">
          Homepage
        </Link>
      </header>
      <figure>
        <img src={character.image} alt={`image of ${character.name}`} />
        <figcaption>
          {character.name}
        </figcaption>
      </figure>
      <button onClick={useBEPost(character)}>Add to Faves</button>
      <button onClick={useBEDelete(character.id)}>Delete from Faves</button>
    </main>
  );
}
